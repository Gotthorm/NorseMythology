	...
	bool SBMObject::Load( const std::string& fileName )
	{
		...
		if( fileName.length() > 0 )
		{
			...
			if( fopen_s( &filePointer, fileName.c_str(), "rb" ) == 0 )
			{
				...
				m_RawData = new unsigned char[ filesize ];

				fread( m_RawData, filesize, 1, filePointer );

				unsigned char * ptr = m_RawData;
				...
				m_RenderData = m_RawData + vertex_data_chunk->data_offset;
				m_Size = vertex_data_chunk->data_size;
				m_Vertices = vertex_data_chunk->total_vertices;
